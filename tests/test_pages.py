...
class PagesContractTests(unittest.TestCase):
    ...
    def test_mobile_and_demo_css_contract(self) -> None:
        ...
        self.assertIn("--budget-progress: 100%", demo_css)
        self.assertIn(".external-deadline-line", demo_css)
        self.assertIn(".browser-timing-summary", demo_css)
        self.assertNotIn("--paper-default-position", demo_css)
        self.assertNotIn(".budget-default-tick", demo_css)
        ...
        self.assertIn("state.data.timing", demo_js)
        self.assertIn("externalDeadlineMs:externalDeadline()", demo_js)
        self.assertIn("internalBudgetMs:item.budgetMs", demo_js)
        self.assertIn('browserWallScope:"single-fptr-wasm-call"', demo_js)
        self.assertIn('paperDeadlineScope:"native-cpp-subprocess-wall"', demo_js)
        self.assertNotIn("state.data.budgets", demo_js)
